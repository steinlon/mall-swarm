...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Override
    public Map<String, Object> generateOrder(final OrderParam orderParam) {
        final List<OmsOrderItem> orderItemList = Lists.newArrayList();
        //获取购物车及优惠信息
        final UmsMember currentMember = memberService.getCurrentMember();
     ...
     * 判断下单商品是否都有库存
     */
    private boolean hasStock(final List<CartPromotionItem> cartPromotionItemList) {
        return cartPromotionItemList.stream()
                .noneMatch(it -> it.getRealStock() == null || it.getRealStock() <= 0);
    }
